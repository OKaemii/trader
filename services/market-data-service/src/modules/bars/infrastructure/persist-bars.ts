...
// write boundary via the platform's single suffix parser.
...
export async function fetchFirstPrintCloses(
  ...
  const keys = bars.map((b) => {
    ...
    return { symbol, market, observation_ts: b.observation_ts, interval };
  });

  // Aggregate latest-first-print per key. The $sort+$group({$first}) pattern picks the
  // smallest knowledge_ts for each (symbol, market, observation_ts) and reads its close.
  const rows = await coll.aggregate([
    { $match: { $or: keys } },
    { $sort: { knowledge_ts: 1 } },
    { $group: {
        _id:   { symbol: '$symbol', market: '$market', observation_ts: '$observation_ts' },
        close: { $first: '$close' },
    } },
  ]).toArray() as Array<{ _id: { symbol: string; market: string; observation_ts: number }; close: number }>;

  for (const row of rows) {
    out.set(`${row._id.symbol}|${row._id.market}|${row._id.observation_ts}`, row.close);
  }
  return out;
}
...
export async function writeBarRevisions(
  ...
  // Pre-compute hashes + split each ticker to its (symbol, market) identity once. Bars carry hashes
  // downstream too in case the validator or audit consumer wants them without re-hashing.
  const incoming = bars
    .filter((b) => Number.isFinite(b.observation_ts))
    .map((bar) => ({ bar, hash: bar.content_hash ?? hashBarContent(bar), id: tickerAdapter.fromT212(bar.ticker) }));
  if (incoming.length === 0) return stats;

  // Single fetch of the current latest revision per (symbol, market, observation_ts).
  // Bounded by the partial-unique index on (symbol, market, observation_ts, interval) filtered
  // by is_superseded:false — sub-ms at universe scale.
  const keys = incoming.map(({ bar, id }) => ({
    symbol: id.symbol,
    market: id.market,
  ...
  const latest = await coll.find(
    ...
    { projection: { _id: 0, symbol: 1, market: 1, observation_ts: 1, content_hash: 1 } },
  ).toArray();
  const latestByKey = new Map<string, string | undefined>(
    latest.map((d: Record<string, unknown>) =>
      [`${d.symbol}|${d.market}|${d.observation_ts}`, typeof d.content_hash === 'string' ? d.content_hash : undefined]),
  );
  ...
  try {
    for (const { bar, hash, id } of incoming) {
        ...
        await session.withTransaction(async () => {
          if (isRevision) {
            await coll.updateMany(
              { symbol: id.symbol, market: id.market, observation_ts: bar.observation_ts, interval, is_superseded: false },
...
// old indexes is wrapped in try/catch; creating the new ones is a no-op if they
// already match.
//
// New layout (Thread A — keyed on the bare identity (symbol, market), not the T212 ticker):
//   - { symbol, market, observation_ts, interval, knowledge_ts } unique  → covers compound
//     uniqueness and the as-of aggregation match.
//   - { symbol, market, observation_ts, interval } partial-unique is_superseded:false →
//     live-read fast lane; exactly one row per logical bar.
//   - { symbol, market, knowledge_ts }                            → revisions admin lookups.
//
// The legacy `ticker_timestamp_interval_unique` AND the prior ticker-keyed `bar_*` indexes are
// dropped first: re-creating an index under the same NAME with a different key spec is a Mongo error,
// so the rename to (symbol, market) keys requires dropping the old definitions. Safe because the
// stores are wiped at cutover — no old-shape rows to keep the old indexes alive for.
export async function ensureBiTemporalIndexes(db: Db): Promise<void> {
  ...
  // Best-effort drop the legacy unique index (new writes don't set `timestamp`, so leaving it would
  // collide every insert on the null tuple) AND the prior ticker-keyed bi-temporal indexes (they are
  // re-created below under the same names but on the (symbol, market) key — a name clash otherwise).
  for (const name of ['ticker_timestamp_interval_unique', 'bar_btemporal_unique', 'bar_latest_unique', 'bar_knowledge_lookup']) {
    try {
      await coll.dropIndex(name);
      log.info(`[market-data] dropped index ${name} (re-created on the symbol+market key where applicable)`);
    } catch (err) {
      // Expected on fresh deploys (index never existed) — log at debug only.
      const msg = err instanceof Error ? err.message : String(err);
      if (!/index not found|ns not found/i.test(msg)) {
        log.warn(`[market-data] index drop failed for ${name} (likely already absent):`, msg);
      }
    }
  }

  await coll.createIndex(
    { symbol: 1, market: 1, observation_ts: 1, interval: 1, knowledge_ts: 1 },
  ...
  await coll.createIndex(
    { symbol: 1, market: 1, observation_ts: 1, interval: 1 },
    {
      unique: true,
  ...
  await coll.createIndex(
    { symbol: 1, market: 1, knowledge_ts: 1 },
