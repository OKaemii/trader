"""NYSE next-session-open calendar — the `knowledge_ts` availability derivation for the PIT lake.

THE PIT CRUX. A filing's raw `accepted_ts` (EDGAR `acceptanceDateTime`, genuine UTC) is WHEN SEC
accepted it, not when a trader could act on it. EDGAR routinely accepts after the 16:00 ET close (a
10-K accepted 18:12 ET Friday). The point-in-time contract is that such a fact only becomes KNOWABLE
at the next session a trader could trade on — Monday's 09:30 ET open, never the Friday it landed after
close. So the lake's `knowledge_ts` = the next NYSE session OPEN at or after the availability instant.
A fact accepted DURING a session (10:00 ET) is knowable the NEXT session's open (you couldn't trade
the open that already passed); a fact accepted strictly BEFORE the open is knowable at that day's open.

LIFTED, not re-derived: the NYSE-session model below is the proven implementation already running in
`services/fundamentals-ingestion/src/normalize/calendar.py` (rule-based holidays, self-contained
UTC→ET DST, next-session-open). It is reproduced here verbatim because that TypeScript-adjacent service
is not importable from quant-core, and quant-core is the single source of truth the harvester +
fundamentals-api + backtest all read. Any correction to the session/holiday rules must edit BOTH copies
(or be hoisted into a shared module) so the live write-path and the replay read-path cannot drift.

WHY a hand-rolled calendar (kept from the original):
  * `@trader/shared-calendar` (the NYSE/LSE `ExchangeCalendar`, `nextOpen`) is TypeScript — NOT reachable
    from Python. quant-core has no Python trading calendar either (its `RebalanceClock` deliberately
    derives month boundaries from the bar stream to stay calendar-free + parity-safe), and
...
# NYSE regular session close, ET wall-clock — used only by the `filed`-date bulk-bootstrap fallback,
# which treats a SEC `filed` date as that day's 16:00 ET close (the conservative public instant) before
# deriving the next session open. The intraday session model otherwise ignores the close (half-days
# move only the close, never the open).
...
def _session_anchor_utc_ms(d: date, hour: int, minute: int) -> int:
    """The `hour:minute` ET wall-clock instant of trading day `d`, as a UTC-ms epoch. The ET→UTC offset
    is resolved at the anchor instant itself (DST is keyed on the actual moment) by a single fixed-point
    step: assume the offset implied by the day's UTC midnight, build the candidate instant, re-resolve
    the offset there and rebuild once. One correction suffices — both the open (09:30) and close (16:00)
    are hours from any DST boundary (02:00 local), so the offset can never flip a second time between
    the candidate and the truth."""
    midnight_utc = datetime(d.year, d.month, d.day, 0, 0, tzinfo=timezone.utc)
    offset = _et_offset_hours(midnight_utc)
    anchor_utc = midnight_utc  # placeholder; overwritten in the loop below
    for _ in range(2):
        anchor_utc = datetime(
            d.year, d.month, d.day, hour, minute, tzinfo=timezone.utc
        ) - timedelta(hours=offset)
        new_offset = _et_offset_hours(anchor_utc)
        if new_offset == offset:
            break
        offset = new_offset
    return int(anchor_utc.timestamp() * 1000)


def _session_open_utc_ms(d: date) -> int:
    """The 09:30 ET open of trading day `d`, as a UTC-ms epoch."""
    return _session_anchor_utc_ms(d, _SESSION_OPEN_HOUR, _SESSION_OPEN_MINUTE)


def _session_close_utc_ms(d: date) -> int:
    """The 16:00 ET close of trading day `d`, as a UTC-ms epoch (the `filed`-fallback public instant)."""
    return _session_anchor_utc_ms(d, _SESSION_CLOSE_HOUR, _SESSION_CLOSE_MINUTE)
...
def derive_knowledge_ts(accepted_ts_ms: int | None, filed: date) -> int:
    """Derive a filing's PIT `knowledge_ts` (UTC ms) — the next NYSE session open at which the fact is
    KNOWABLE — from its EDGAR availability inputs.

    Two paths, by which input the harvester has (decomposition note 3):

      * **Acceptance known (the 30-min sweep path).** The sweep fetches `/submissions` per filed CIK, so
        each accession carries its real `acceptanceDateTime`. `knowledge_ts = next_session_open_ms(
        accepted_ts)` — the precise next-session-open after the exact accept instant (an after-hours
        18:12-ET-Friday accept → Monday's open).

      * **Acceptance absent (the bulk-zip bootstrap path).** The 15k-CIK bulk download does NOT carry
        per-accession acceptance times (15k `/submissions` calls would defeat the bulk download), so
        only the SEC `filed` DATE is available. We treat `filed` conservatively as that day's 16:00 ET
        CLOSE — already the SEC public date, so deriving the next session open after the close is
        LOOK-AHEAD-SAFE (never earlier than acceptance-precise would have been): a fact filed on a
        trading day becomes knowable at the NEXT session's open, a Friday-filed fact at Monday's open. A
        later sweep refines the bulk-written CIK with the exact `accepted_ts`, sharpening (never
        loosening) the availability.

    `filed` is a `date32` column (the SEC filing date); `accepted_ts_ms` is the nullable acceptance
    UTC-ms (`None` on the bulk path). Always returns a non-null UTC-ms epoch (the lake's `knowledge_ts`
    column is NON-null — the PIT read axis).
    """
    if accepted_ts_ms is not None:
        return next_session_open_ms(accepted_ts_ms)
    # Bulk fallback: anchor `filed` to that day's NYSE close (the conservative public instant), then
    # roll to the next session open. Using the close (not midnight/open) guarantees the result is the
    # NEXT trading session, never the filed day itself — strictly look-ahead-safe.
    filed_close_ms = _session_close_utc_ms(filed)
    return next_session_open_ms(filed_close_ms)
