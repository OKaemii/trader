 ...
 */
export interface BoundContext<C extends InternalContract<ZodTypeAny | undefined, ZodTypeAny | undefined, ZodTypeAny | undefined>> {
    c: Context;
    params: InferParams<C>;
    body: InferRequest<C>;
}

export type ContractHandler<C extends InternalContract<ZodTypeAny | undefined, ZodTypeAny | undefined, ZodTypeAny | undefined>> = (
    ctx: BoundContext<C>,
...
>(
    ...
    const wrapped: MiddlewareHandler = async (c) => {
        ...
        const result = await handler({ c, params, body });
